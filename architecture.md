...
A second, deliberately tiny **post-production job** (`hn-shownotes`, [shownotes/](shownotes/)) runs after the audio pipeline and never touches it. It is optional: the pipeline is complete without it, and the video edition is opt-in via `VIDEO=1` (off by default, since Veo is the dominant cost). It reads the published script, has **Gemma** (gemma-4-31b-it through the Gemini API) write the listener-facing episode description into the feed, maps each story's start time from the audio (Gemini audio understanding, snapped to the silences the pipeline inserts between TTS segments), splits each story evenly into roughly ten-second windows, has Gemini summarize what is said in each window and Gemma write one scene per story plus a per-window action, renders one 8-second clip per window with **Veo** (veo-3.1-fast through Vertex AI), and stitches them slowed under the episode audio so the visuals follow the conversation. Veo dominates the cost at ~US$0.15 per second of output, roughly US$60 for an 8-9 minute episode. Any failure leaves the feed exactly as the pipeline published it.
